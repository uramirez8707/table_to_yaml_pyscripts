...
def check_gridname(grid_name):
    ...
    if (gridname not in valid): sys.exit('ERROR: The only values allowed for grid_name are "OCN", "LND", "ATM", "ICE"')

def check_fieldname_code(fieldname):
    if (fieldname == ""): sys.exit("ERROR: Fieldname is not empty")

def check_filename_and_field(field, interp_method):
    if (field =="" and interp_method != "none"): sys.exit('ERROR: If "fieldname_file" is empty, interp_method must be "none"')

def check_interp_method(interp_method):
    valid = ["bilinear", "bicubic", "none"]
    if (interp_method not in valid): sys.exit('ERROR: The only values allowed for interp method are "bilinear", "bicubic", and "none"')

def check_region_type(region_type):
    valid = ["inside_region", "outside_region"]
    if (region_type not in valid): sys.exit('ERROR: The only values allowed for region_type are "inside_region" and "outside_region"')

def check_if_bounds_present(entry):
    if ("lat_start" not in entry): sys.exit('ERROR: lat_start must be present if region_type is set')
    if ("lat_end" not in entry): sys.exit('ERROR: lat_end must be present if region_type is set')
    if ("lon_start" not in entry): sys.exit('ERROR: lon_start must be present if region_type is set')
    if ("lon_end" not in entry): sys.exit('ERROR: lon_end must be present if region_type is set')

def check_region(my_type, start, end):
    if (start > end): sys.exit('ERROR: '+my_type+"_start is greater than "+my_type+"_end")
